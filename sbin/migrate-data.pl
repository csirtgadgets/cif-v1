...
sub _process_message {
    ...
    unless($data->{'data'} =~ /^\{\"xmlns\:xsi/){
        debug('skipping: '.$data->{'id'}) if($debug > 4);
...
sub _writer_routine {
    ...
    my $dbi = CIF::Archive->new({ config => $config });
    ...
    my $sth2 = CIF::Archive->sql_custom2();
    ...
    do {
        ...
        if($poller->has_event('writer')){
            ...
            if($msg ne '-1'){
                ...
                ($err,$ret) = CIF::Archive->insert_index({ 
                    data        => $msg->{'data'},
                    feeds       => $dbi->{'feeds'},
